...
AAlliance_Hunter::AAlliance_Hunter() {
	...
	static ConstructorHelpers::FClassFinder<UAnimInstance> AnimationBlueprint(TEXT("AnimBlueprint'/Game/Units/Alliance/StylizedHumanMale/Animations/AllianceHunterAnimation_BP.AllianceHunterAnimation_BP_C'"));
	if (AnimationBlueprint.Succeeded()) {
		GetMesh()->AnimClass = AnimationBlueprint.Class;
	}
}

void AAlliance_Hunter::BeginPlay() {
	Super::BeginPlay();
	MovementComponent->MaxWalkSpeed = 250;
}


void AAlliance_Hunter::SheathMainHand()
{
...
void AAlliance_Hunter::EquipMainHand()
{
	Super::EquipMainHand();
	//if (MainHandItem) {
